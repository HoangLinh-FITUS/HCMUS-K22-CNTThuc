...
namespace calculator {
    ...
    struct BigHex {
        ...
        BigHex& operator %= (const BigHex& otherNumber) {
            ...
            Spw2 = min(Spw2, D * this->size() + 1); // (2^D)^i.... 0 
...
int main(int argc, char**argv) {
    ...
    bool N_isOdd = BigHex<28>(N).is_odd();
    ...
    for (int i = 0; i < x; i++) {
        ...
        if (!N_isOdd) {
            ...
            pw = MONTGOMERY::powerMod(m[i], e, N);
        }
        
        string mi = pw.to_str();
        if (pos.find(mi) != pos.end()) Fout << pos[mi] << " ";
        else Fout << "-1 ";
    }
