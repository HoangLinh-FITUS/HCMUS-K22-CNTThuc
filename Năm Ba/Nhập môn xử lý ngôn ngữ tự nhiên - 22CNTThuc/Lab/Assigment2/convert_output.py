...
RED_COLOR, DEFAULT_COLOR, BLUE_COLOR, GREEN_COLOR = 0, 1, 2, 3
...
class AlignmentWithBbox:
    ...
    def matching_by_cnt_word(self) -> list[dict]:
        ...
        returns:
            ...
            => (sentence_ocr, id_match, points, id_sentence) 
            tuong ung voi:
            + cau ma ocr ra duoc 
            + vi tri cau quoc ngu ma cau ocr co the alignment 
                neu -1 tuc la cau nay k co cau de aligment 
            + toa do cua bbox 
            + thu tu cau ocr ra duoc
        '''
...
class AligmentWithWord(AlignmentWithBbox):
    ...
    def _create_dictionary(self):
        ...
        returns:
            + tu dien nhung cua tu han ngu giong nhau
            + tu dien quoc ngu (dict)
        '''
        file_sinoNom = pd.read_excel(self._FILE_SINONOM_SIMILAR_DIC) # lay ra du lieu sinom trong file chi dinh 
        file_quocNgu = pd.read_excel(self._FILE_QUOCNGU_SINONOM_DIC) # lay ra du lieu quocngu trong file chi dinh 
        ...
        return sinoNom_similar_dic, quocngu_sinoNom_dic
        
    def __init__(self, ocr_matched: list[dict], text_of_file: list[str]):
        self.ocr_matched = ocr_matched
        self.text_of_file = text_of_file
        self.sinoNom_similar_dic, self.quocngu_sinoNom_dic = self._create_dictionary()
    ...
    def _get_sinoNom_similar_dic(self, word: str) -> list[str]:
        ...
        '''
        if word in self.sinoNom_similar_dic:
            return [word] + ast.literal_eval(self.sinoNom_similar_dic[word])
        return [word]
    ...
    def _get_quocngu_sinoNom_dic(self, word: str) -> list[str]:
        ...
        '''
        word = word.lower()
        if word not in self.quocngu_sinoNom_dic:
            return []
        
        return self.quocngu_sinoNom_dic[word.lower()]
    ...
    def _cmp_word(self, word_HN, word_QN) -> int:
        ...
        if word_HN in self.SPECIAL_CHARS: return DEFAULT_COLOR 

        if word_QN in self.SPECIAL_CHARS: return DEFAULT_COLOR 
...
def sort_bbox(data: list[Bbox]):
    ''' xep xep tu tren xuong duoi , tu trai sang phai 

    args:
        data: danh sach nhung bbox 
    '''
...
def extract_text_in_file(
    ...
    ''' trich nhung noi dung trong file excel

    args:
        file_excel: ten file excel trich xuat
    ...
    TEXT_PDF = pd.read_excel(file_excel)

    sentences_HN, sentences_QN = [], []
    col_page_index, col_text_HN, col_text_QN = TEXT_PDF.keys()
    
    for page_id, sentence_HN, sentence_QN in zip(
        TEXT_PDF[col_page_index].values, 
        TEXT_PDF[col_text_HN].values, 
        TEXT_PDF[col_text_QN].values
    ):
        if start_page < int(page_id) and int(page_id) < stop_page:
            #print(type(sentence_HN))
            if type(sentence_HN) == float: sentence_HN = '' # neu sentence_HN = Nan 
            if type(sentence_QN) == float: sentence_QN = ''
            sentences_HN.append(sentence_HN)
            sentences_QN.append(sentence_QN)
    
    return sentences_HN, sentences_QN
...
def write_output(
    ...
    row = start_row
    name_image, page_index = name_image.split('.')
    id_sentence_ocr = 1
...
def solve(
    ...
    return results, text_HN, text_QN

def extract_Label(file_path: str) -> list[tuple]:
    ''' trich xuat noi dung ma PProlabel tra ra 

    args:
        file_path: dung dan cua file Label.txt
    returns:
        (name_imgage, page_index, data_ocr)
    '''
    data_img = []
    
    with open(file_path, 'r') as file_label:
        data = file_label.read().split('\n')
        ...
        for [name_image, data_ocr] in data:
            ...
            data_img.append((name_image, int(page_index), data_ocr)) 

    return data_img

def main(row_output_xlsx: int = 1) -> int:
    # hang bat dau trong file output.xlsx

    data_img = extract_Label(_FOLDER + 'Label.txt')

    next_page_index = [int(page_index) for _, page_index, __ in data_img]
    next_page_index.append(int(1e9))

    id_next_page = 0
    for name_image, page_index, data_ocr in data_img:

        id_next_page += 1

        start_page_index = page_index
        stop_page_index = next_page_index[id_next_page]
        
        results, text_HN, text_QN = solve(name_image, start_page_index, stop_page_index, data_ocr)
...
_FOLDER = 'dang1/'
_FILE_NAME = 'text_Bondaytrecon.xlsx'
main(row)
